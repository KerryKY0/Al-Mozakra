...
const PdfPage = (props: {
  ...
  const { pageNumber, scale, rotation } = props;
  const { t } = useI18n();
  ...
  return (
    <div ref={ref} className="w-full flex justify-center min-h-[500px]">
      {isVisible ? (
        <Page
          ...
          loading={
            <div className="w-full h-[500px] flex items-center justify-center text-muted">
              {t("loadingPage")}
            </div>
          }
        />
...
const FileViewer: React.FC = () => {
  ...
  useEffect(() => {
    const loadPdfData = async () => {
      ...
      // Pass URL directly to react-pdf for better performance (range requests)
      setPdfData(file.contentUrl);
      setIsBuffering(false);
    };
    loadPdfData();
  }, [file?.contentUrl, file?.type]);

  if (!file) return <div className="text-center p-10">{t("fileNotFound")}</div>;
  ...
  return (
    <div
      className="flex flex-col h-full max-w-5xl mx-auto space-y-4 md:space-y-6 pb-4 pt-2 md:pt-0"
      dir={lang === "ar" ? "rtl" : "ltr"}
    >
      <button
        onClick={handleBack}
      ...
      >
        ...
        {isDev ? t("backToFiles") : t("backToSubject")}
      </button>
      ...
      >
          ...
          <div className="flex items-center gap-1 md:gap-2 shrink-0">
            <div className="flex items-center gap-1 bg-space-900 rounded-lg p-1 border border-space-700 hidden sm:flex">
              <button
                ...
                className="p-1 hover:bg-space-700 rounded text-white"
                title={t("zoomOut")}
              >
                <ZoomOut size={18} />
              </button>
              <span className="text-xs text-muted w-10 text-center font-mono">
            ...
            {file.type === FileType.PDF && (
              <button
                ...
                className="p-2 bg-space-900 rounded-lg text-white hover:text-space-accent border border-space-700 transition-colors"
                title={t("rotate")}
              >
                <RotateCw size={18} />
              </button>
        ...
        <div className="relative flex-1 bg-black/20 overflow-auto custom-scrollbar">
          <div
            className={`min-h-full flex items-start justify-center p-2 md:p-4 ${file.type !== FileType.PDF ? "items-center" : ""}`}
          >
              ...
              <>
                {isBuffering ? (
                  <div className="flex flex-col items-center justify-center h-full w-full mt-20">
                    ...
                    <p className="text-muted">{t("loadingData")}</p>
                  </div>
                ) : errorMsg ? (
      ...
      <div className="p-4 md:p-6 mb-20">
          ...
          {file.comments && file.comments.length > 0 ? (
            file.comments.map((c) => {
              ...
              return (
                <div
                  key={c.id}
                  className="bg-space-900/50 p-3 rounded-lg border border-space-700/50 group relative"
                >
                  <div className="flex justify-between items-start mb-1">
                    <span className="font-bold text-space-accent text-xs md:text-sm">
                      {c.userName}
                    </span>
                  ...
                  {canManage && !editingCommentId && (
                    <div className="absolute top-2 left-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        className="p-1 bg-space-800 rounded text-blue-400 hover:bg-space-700"
                      >
                        <Edit size={12} />
                      </button>
                      <button
                        onClick={() => setDeleteConfirmId(c.id)}
                        className="p-1 bg-space-800 rounded text-red-400 hover:bg-space-700"
                      >
                        <Trash2 size={12} />
                      </button>
                    </div>
        ...
        <div className="flex gap-2 items-center">
          ...
          <button
            ...
            className="bg-space-accent text-space-900 p-2 rounded-lg hover:bg-yellow-400 disabled:opacity-50 h-10 w-10 flex items-center justify-center shadow-md"
          >
            <Send size={18} />
          </button>
        </div>
          ...
          <div className="bg-space-800 rounded-2xl border border-red-500 w-full max-w-sm shadow-2xl p-6 text-center">
            ...
            <div className="flex gap-3 justify-center">
              <button
                ...
                className="bg-red-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-red-500"
              >
                {t("yesDelete")}
              </button>
              <button
                onClick={() => setDeleteConfirmId(null)}
                className="bg-space-700 text-white px-6 py-2 rounded-lg font-bold hover:bg-space-600"
              >
                {t("cancel")}
              </button>
            </div>
