...
const GlobalChat: React.FC = () => {
  ...
  const [viewersModalMsgId, setViewersModalMsgId] = useState<string | null>(
    null,
  );
  ...
  const isBanned =
    currentUser && chatSettings.bannedUsers.includes(currentUser.id);
  ...
  useEffect(() => {
    if (chatMessages.length > prevMessagesLen.current) {
      ...
      if (!isOpen && currentUser && lastMsg?.userId !== currentUser.id) {
  ...
  useEffect(() => {
    if (isAiMode && aiMessages.length === 0) {
      const greeting: ChatMessage = {
        id: "ai-init",
        userId: "ai-bot",
        userName: t("smartAssistant"),
        userRole: UserRole.ADMIN,
        content: t("aiGreeting").replace("{name}", currentUser?.name || ""),
        type: "TEXT",
    ...
    try {
      ...
      const contents = [
        ...historyParts,
        {
          ...
          parts: [
            {
              text: `Answer following the instructions and context below.
User Question: ${userMsg}

Instructions:
- Respond strictly in the following language: ${lang === "ar" ? "Arabic (Modern Standard Arabic preferred, Egyptian dialect acceptable)" : "English"}.
- Tone: Helpful, friendly, and professional.
- Context: You are "المساعد الذكي" on the student platform "المذاكرة".
...
- Files: ${filesContext}`,
      ...
      const apiKey =
        ...
        "AIzaSyA126x7s1CN1kFiDfOJMuT7cMlPjHEplWQ";
      const ai = new GoogleGenAI({ apiKey });

      // Using a correct model name for Gemini
      const response = await ai.models.generateContent({
        model: "gemini-1.5-flash",
        contents: contents,
      });

      const aiText = response.text || t("aiConnectionError");
      ...
      if (isAiMode) {
        ...
        if (msgIndex !== -1) {
          ...
          if (
            ...
            newMessages[msgIndex + 1].userId === "ai-bot"
          ) {
          ...
          const historyBefore = newMessages
            .slice(0, msgIndex)
  ...
  const saveForbiddenWords = () => {
    ...
    showToast(t("wordFilteredUpdated"));
  };
  ...
  const getPreviewMessages = () => {
    return chatMessages.filter((m) => m.userId !== currentUser.id).slice(-3); // Get last 3
  };
      ...
      <div className="fixed bottom-6 left-6 z-[100] flex flex-col items-start gap-4 group">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`relative group p-4 rounded-full bg-space-900 border-2 transition-all duration-300 hover:scale-110 flex items-center justify-center overflow-hidden ${
              unreadCount > 0
                ? "animate-notification-shake animate-glow-red border-red-500 shadow-[0_0_20px_rgba(239,68,68,0.4)]"
                : "border-space-accent hover:border-white shadow-[0_0_20px_rgba(253,184,19,0.2)] hover:shadow-[0_0_30px_rgba(253,184,19,0.4)] shadow-sm"
            }`}
          >
            <div className="absolute inset-0 bg-gradient-to-tr from-space-accent/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
            <MessageCircle
              size={26}
              className={`${unreadCount > 0 ? "text-red-500" : "text-space-accent"} group-hover:text-white relative z-10 transition-colors`}
            />
            <Sparkles
              size={14}
              className="text-white absolute top-2 right-2 opacity-0 group-hover:opacity-100 animate-pulse"
            />
            {unreadCount > 0 && (
              <div className="absolute -top-1 -right-1 bg-red-600 text-white text-[10px] font-bold h-5 min-w-[20px] px-1 rounded-full flex items-center justify-center border-2 border-space-900 shadow-sm z-50 font-sans">
                {unreadCount > 99 ? "99+" : unreadCount}
              </div>
            )}
          </button>
  ...
  return (
    <div className={containerClasses}>
      ...
      <div className="bg-space-900 p-3 border-b border-space-700 flex justify-between items-center shrink-0">
        <div className="flex items-center gap-2">
          <div
            className={`w-2 h-2 ${isAiMode ? "bg-blue-500" : "bg-green-500"} rounded-full animate-pulse`}
          ></div>
          <span className="font-bold text-main">
            {isAiMode ? t("smartAssistant") : t("groupChat")}
          </span>
          {!isAiMode && chatSettings.isLocked && (
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            title={isAiMode ? t("groupChat") : t("smartAssistant")}
          >
          ...
          {isDev && !isAiMode && (
            <div className="relative">
              <button
                ...
                className="p-1 hover:bg-space-700 rounded text-muted"
              >
              ...
              {showOptions && (
                <div className="absolute top-8 left-0 bg-space-800 border border-space-700 rounded-lg shadow-xl w-48 p-2 z-50">
                  <button
                    ...
                    className="flex items-center gap-2 w-full p-2 hover:bg-space-700 rounded text-xs text-main"
                  >
                    ...
                    {chatSettings.isLocked ? t("openChat") : t("closeChat")}
                  </button>
                  <button
                    onClick={() => setForceHideNames(!forceHideNames)}
                    className="flex items-center gap-2 w-full p-2 hover:bg-space-700 rounded text-xs text-main"
                  >
                    {forceHideNames ? <Eye size={14} /> : <EyeOff size={14} />}{" "}
                    {forceHideNames ? t("showNames") : t("hideNames")}
                  </button>
                  <button
                    onClick={() => {
                      setShowWordFilterModal(true);
                      setShowOptions(false);
                    }}
                    className="flex items-center gap-2 w-full p-2 hover:bg-space-700 rounded text-xs text-main"
                  >
                    <Ban size={14} /> {t("forbiddenWords")}
                  </button>
                  <button
                    onClick={() => {
                      clearChat();
                    ...
                    className="flex items-center gap-2 w-full p-2 hover:bg-red-500/20 text-red-400 rounded text-xs"
                  >
                    <Trash2 size={14} /> {t("clearChatText")}
                  </button>
                </div>
              )}
            </div>
      ...
      {/* Messages Body */}
      <div
        className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar bg-black/20 flex flex-col"
        onClick={() => {
      ...
      >
        ...
        {currentMessages.map((msg) => {
          ...
          const showName =
            isBot || isDev || (!msg.isAnonymous && !forceHideNames) || isMe;
          const sender = isBot
            ? { avatarUrl: null, name: t("smartAssistant") }
          ...
          const activeReactions = Object.entries(msg.reactions).filter(
            ([_, users]) => (users as string[]).length > 0,
          );
          const visibleReactions = activeReactions.slice(0, 4);
          const displayedEmojis = isPickerExpanded
            ? emojis
            : emojis.slice(0, 4);
          ...
          return (
            <div
              key={msg.id}
              className={`flex gap-3 ${isMe ? "flex-row" : "flex-row-reverse"} items-end w-full mb-4 group/row`}
            >
              <div
              ...
              <div
                className={`flex flex-col ${isMe ? "items-start" : "items-end"} max-w-[80%] relative`}
              >
                {showName && (
                ...
                <div className="relative group">
                  ...
                  >
                      ...
                      <>
                        ...
                        {msg.type === "IMAGE" && (
                          <img
                            src={msg.content}
                            alt="shared"
                            className="rounded-xl max-h-60 object-cover shadow-lg hover:scale-[1.02] transition-transform cursor-pointer"
                          />
                        )}
                        {msg.type === "VIDEO" && (
                          <video
                            src={msg.content}
                            controls
                            className="rounded-xl max-h-60 w-full shadow-lg"
                          />
                        )}
                        {msg.type === "AUDIO" && (
                          <div className="shadow-lg rounded-full overflow-hidden">
                            <audio
                              src={msg.content}
                              controls
                              className="w-full min-w-[250px] bg-space-800"
                            />
                          </div>
                        )}
                        {msg.isEdited && (
                          <span
                            className={`text-[9px] opacity-60 block text-left mt-1 italic ${isMedia ? "text-white drop-shadow-md" : ""}`}
                          >
                            {t("editedMsg")}
                          </span>
                        )}
                    ...
                    {activeReactions.length > 0 && (
                      <div
                        className={`absolute -bottom-5 left-1/2 transform -translate-x-1/2 flex items-center gap-1 rounded-full px-1.5 py-0.5 shadow-lg z-10 min-w-max ${isMedia ? "bg-black/70 backdrop-blur-sm border border-white/10" : "bg-space-800"}`}
                      >
                        {visibleReactions.map(([emoji, users]) => {
                          const isReactedByMe = (users as string[]).includes(
                            currentUser.id,
                          );
                          return (
                            <button
                              ...
                              className={`flex items-center gap-0.5 px-1 rounded-full transition-all animate-fade-in ${isReactedByMe ? "bg-space-accent/20 text-space-accent" : "hover:bg-space-700 text-muted"}`}
                            >
                  ...
                  {showMenuButton && (
                    <div
                      className={`absolute top-1/2 transform -translate-y-1/2 ${isMe ? "-left-14" : "-right-14"} z-20 opacity-0 group-hover:opacity-100 transition-opacity`}
                    >
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          setMessageMenuId(
                            messageMenuId === msg.id ? null : msg.id,
                          );
                          setActiveMessageId(null);
                        }}
                  ...
                  {activeMessageId === msg.id && (
                    <div
                      className={`absolute bottom-full mb-2 ${isMe ? "right-0" : "left-0"} bg-space-800 border border-space-700 rounded-full p-1.5 flex flex-col md:flex-row gap-1 z-50 shadow-2xl animate-fade-in w-max`}
                    >
                      ...
                      {!isPickerExpanded && (
                        <button
                          ...
                          className="hover:scale-125 transition-transform p-1 text-sm leading-none flex items-center justify-center text-muted hover:text-white"
                        >
                          <Plus size={14} />
                        </button>
                      )}
      ...
      {(!chatSettings.isLocked || isDev || isAiMode) && !isBanned ? (
          ...
          {!isDev && !isAiMode && (
            <div className="flex items-center gap-2 mb-2 px-2">
              <input
                ...
                className="accent-space-accent w-3 h-3"
              />
          ...
          <div className="flex items-center gap-2">
            {!isAiMode && (
              <button
                ...
                className="text-muted hover:text-space-accent p-2 hover:bg-space-800 rounded-full transition-colors"
              >
                <Paperclip size={20} />
              </button>
            )}
            ...
            <div className="flex-1 relative">
              <input
                ...
                placeholder={isAiMode ? t("askAssistant") : t("writeMessage")}
        ...
        <div className="bg-space-900 p-4 text-center text-xs text-red-400 border-t border-space-700 font-bold shrink-0">
          {isBanned ? t("bannedFromChat") : t("chatClosedByAdmin")}
        </div>
      )}
          ...
          <div className="bg-space-800 border border-space-700 rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden flex flex-col max-h-[80%]">
            <div className="p-4 border-b border-space-700 flex justify-between items-center bg-space-900">
              <h3 className="font-bold text-main flex items-center gap-2">
                <Eye size={18} className="text-blue-400" /> {t("viewedBy")}
              </h3>
              <button onClick={() => setViewersModalMsgId(null)}>
                <X size={18} className="text-muted hover:text-white" />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-2 custom-scrollbar">
              {getViewersForMessage(
                chatMessages.find((m) => m.id === viewersModalMsgId)!,
              ).length > 0 ? (
                getViewersForMessage(
                  chatMessages.find((m) => m.id === viewersModalMsgId)!,
                ).map((viewer) => (
                  <div
                    key={viewer.id}
                    className="flex items-center gap-3 p-3 hover:bg-space-700/50 rounded-lg transition-colors border-b border-space-700/30 last:border-0"
                  >
                    {viewer.avatarUrl ? (
                      <img
                        src={viewer.avatarUrl}
                        alt=""
                        className="w-8 h-8 rounded-full object-cover border border-space-600"
                      />
                    ) : (
          ...
          <div className="bg-space-800 border border-space-700 rounded-2xl w-64 shadow-2xl overflow-hidden relative">
            ...
            <div className="flex flex-col items-center pt-6 pb-4 bg-gradient-to-b from-space-700 to-space-800">
              {viewingChatUser.avatarUrl ? (
                <img
                  src={viewingChatUser.avatarUrl}
                  alt=""
                  className="w-20 h-20 rounded-full object-cover border-2 border-space-accent shadow-lg mb-2"
                />
              ) : (
              ...
              <h3 className="font-bold text-main text-lg">
                {viewingChatUser.name}
              </h3>
              <span className="text-xs text-muted">
                {viewingChatUser.role === UserRole.ADMIN
                  ? t("mainDev")
                  : viewingChatUser.role === UserRole.SUB_ADMIN
                    ? t("subDev")
                    : t("student")}
              </span>
              {isDev && viewingChatUser.id !== currentUser.id && (
                ...
                >
                  <Ban size={12} />{" "}
                  {chatSettings.bannedUsers.includes(viewingChatUser.id)
                    ? t("unban")
                    : t("banFromChat")}
                </button>
              )}
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="flex items-center gap-3 text-sm">
                ...
                <span className="text-main">
                  {viewingChatUser.gender === "MALE"
                    ? t("male")
                    : viewingChatUser.gender === "FEMALE"
                      ? t("female")
                      : t("unspecified")}
                </span>
              </div>
              <div className="flex items-center gap-3 text-sm">
                ...
                <span className="text-main">
                  {viewingChatUser.birthDate || t("unspecified")}
                </span>
              </div>
              <div className="flex items-center gap-3 text-sm">
                ...
                <span className="text-main text-xs">
                  {t("joined")}{" "}
                  {viewingChatUser.createdAt
                    ? new Date(viewingChatUser.createdAt).toLocaleDateString(
                        lang === "ar" ? "ar-EG" : "en-US",
                      )
                    : t("unspecified")}
                </span>
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-space-800 border border-space-700 rounded-2xl w-full max-w-sm shadow-2xl p-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold text-main flex items-center gap-2">
                <Ban size={18} className="text-red-400" /> {t("forbiddenWords")}
              </h3>
              <button onClick={() => setShowWordFilterModal(false)}>
                <X size={18} className="text-muted" />
              </button>
            </div>
            <p className="text-xs text-muted mb-2">{t("addForbiddenWords")}</p>
            <textarea
            ...
            <button
              ...
              className="w-full bg-space-accent text-space-900 font-bold py-2 rounded hover:bg-yellow-400"
            >
              {t("save")}
            </button>
          </div>
